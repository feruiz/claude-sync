...
- `~/.claude.json` → copy + merge (jq) — **has merge, lacks conflict detection**
...
1. **Conflict detection on claude.json** — detect conflicts in object fields (mcpServers) on push, interactive prompt when TTY, local wins silently in automation. Prevents data loss in the existing mechanism.
